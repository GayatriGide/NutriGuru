...
non_veg_keywords = ['Tuna', 'Chicken', 'Salmon', 'Goat', 'Rabbit', 'Pork', 'Bacon', 'Shrimp',
                    'Meatballs', 'Beef', 'Turkey', 'Oyster', 'Egg', 'Yolk']
...
df['translated'] = df['description'].apply(translate_description)
...
def search_food_names_without_allergy(df_filtered, predicted_food, search_words):
    # Initialize sets to keep track of printed food names and search words
    printed_food_names = set()
    printed_search_words = set()
    
    for item in predicted_food:
        printed_search_words.add(item)
    
    for _, row in df_filtered.iterrows():
        # Translate the description of the food item
        translated_description = row['translated']
...
def get_description(df, food_items):
    descriptions = []
    for food_item in food_items:
        ...
        if matched_food:
            ...
            if score >= 80:  # You can adjust the threshold as needed
                ...
                description = df.loc[df['Name'] == matched_food_name, 'translated'].values[0]
...
def search_food_names_with_allergy(df_filtered,predicted_food, search_words, allergies):
    # Initialize sets to keep track of printed food names and search words
    printed_food_names = set()
    printed_search_words = set()
    ...
    # Initialize allergy_found variable before the loop
    allergy_found = False
    ...
    for _, row in df_filtered.iterrows():
        ...
        if translated_description is not None:
            # Reset allergy_found for each row
            allergy_found = False
            ...
            # Check if allergy is found before adding to printed_food_names
...
def create_pie_chart(food, predicted_food_names):
    ...
    # Perform fuzzy string matching to find the closest match
        ...
        if match[1] >= 80:  # Adjust the threshold as needed
            ...
            vegnonveg = item_row['VegNonVeg'].values[0]
            if vegnonveg == 'Veg':
            ...
            # Convert the plot to HTML content
            buffer = io.BytesIO()
            plt.savefig(buffer, format='png', bbox_inches='tight')  # Use bbox_inches='tight' to ensure that the entire content is captured
            buffer.seek(0)
            image_base64 = base64.b64encode(buffer.read()).decode('utf-8')
            pie_chart_html = f'<img src="data:image/png;base64,{image_base64}" alt="{food_item} Pie Chart">'
            plt.close()  # Close the plot to prevent overlapping
            
            pie_charts.append({
                'chartId': food_item.replace(' ', '_'),  # Replace spaces with underscores for chart ID
                'chartData': pie_chart_html
            })
